                ...
                <div class="card">
                    ...
                    <div class="card-body text-center drill-body">
                        ...
                        <template v-if="isStarted && !isCountDown && !isEnd">
                            <p>{{timerNum}}</p>
                            <span v-for="(word, index) in problemWords" :class="{'text-primary': index < currentWordNum}" :key="index">{{word}}</span>
                        </template>
...
export default {
    ...
    methods: {
        ...
        countDown: function () {
            // 効果音読み込み
            const countSound = new Audio('../sounds/button06.mp3')
            const startSound = new Audio('../sounds/button06.mp3')
        ...
        showFirstProblem: function () {
            ...
            // 効果音読み込み
            const okSound = new Audio('../sounds/button06.mp3')
            const ngSound = new Audio('../sounds/button06.mp3')
            const nextSound = new Audio('../sounds/button06.mp3')
        ...
        countTimer: function () {
            ...
            const endSound = new Audio('../sounds/button06.mp3')
            console.log('セットインターバル開始');
            let timer = window.setInterval(() => {
                console.log('タイマー１');
                this.timerNum -= 1
                console.log('タイマー２')
